...
def get_options(argv):
    ...
    parser.add_argument(
        ...
        type=int,
        dest="ymin",
        default=default_values["ymin"],
        metavar="YMIN",
        help=("use YMIN min value for Y (default: %i)" % default_values["ymin"]),
    )
    parser.add_argument(
        "--ymax",
        action="store",
        type=int,
        dest="ymax",
        default=default_values["ymax"],
        metavar="YMAX",
        help=("use YMAX max value for Y (default: %i)" % default_values["ymax"]),
    )
    parser.add_argument(
        "--umin",
        action="store",
        type=int,
        dest="umin",
        default=default_values["umin"],
        metavar="UMIN",
        help=("use UMIN min value for U (default: %i)" % default_values["umin"]),
    )
    parser.add_argument(
        "--umax",
        action="store",
        type=int,
        dest="umax",
        default=default_values["umax"],
        metavar="UMAX",
        help=("use UMAX max value for U (default: %i)" % default_values["umax"]),
    )
    parser.add_argument(
        "--vmin",
        action="store",
        type=int,
        dest="vmin",
        default=default_values["vmin"],
        metavar="VMIN",
        help=("use VMIN min value for V (default: %i)" % default_values["vmin"]),
    )
    parser.add_argument(
        "--vmax",
        action="store",
        type=int,
        dest="vmax",
        default=default_values["vmax"],
        metavar="VMAX",
        help=("use VMAX max value for V (default: %i)" % default_values["vmax"]),
    )
        ...
        def __call__(self, parser, namespace, values, option_string=None):
            ...
            for v in VALUE_LIST_RANGE:
                ...
                setattr(
                    namespace,
                    v,
                    predefined_images[namespace.predefined][namespace.range][v],
                )
    ...
    parser.add_argument(
        ...
        type=str,
        dest="ygrad",
        default=default_values["ygrad"],
        choices=GRAD_LIST,
        metavar="YGRAD",
        help=("y gradient %r (default: %s)" % (GRAD_LIST, default_values["ygrad"])),
    )
    parser.add_argument(
        "--ugrad",
        action="store",
        type=str,
        dest="ugrad",
        default=default_values["ugrad"],
        choices=GRAD_LIST,
        metavar="UGRAD",
        help=("u gradient %r (default: %s)" % (GRAD_LIST, default_values["ugrad"])),
    )
    parser.add_argument(
        "--vgrad",
        action="store",
        type=str,
        dest="vgrad",
        default=default_values["vgrad"],
        choices=GRAD_LIST,
        metavar="VGRAD",
        help=("v gradient %r (default: %s)" % (GRAD_LIST, default_values["vgrad"])),
    )
    parser.add_argument(
        "-o",
        "--outfile",
